...
class TestCICDWorkflows:
    ...
    def test_ci_cd_workflow_structure(self):
        ...
        assert (
            "push" in triggers or "workflow_dispatch" in triggers
                            ...
                            if isinstance(step, dict) and "uses" in step:
                                ...
                                if "@" in action:
                                    ...
                                    if version in ["main", "master"]:
                                        print(
                                            f"Warning: {workflow_file.name} uses {action} "
                ...
                for i, line in enumerate(lines):
                    if "${{ secrets." in line and "echo" in line.lower():
                        ...
                        if not stripped.startswith("#"):
                            print(
                                f"Warning: Potential secret exposure in {workflow_file.name}:{i+1}"
                            )
...
class TestWorkflowQuality:
    ...
    def test_workflow_naming_conventions(self):
        ...
        for workflow_file in workflow_files:
            ...
            assert (
                filename.replace("-", "").replace("_", "").isalnum()
                        ...
                        for step in python_setup_steps:
                            if "with" in step:
                                ...
                                if "cache" not in step_config:
                                    print(
                                        f"Performance tip: {workflow_file.name} job '{job_name}' "
...
pytestmark = [pytest.mark.cicd, pytest.mark.infrastructure]
