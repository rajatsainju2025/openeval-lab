...
class ProjectHealthDashboard:
    ...
    def run_command(
        ...
        try:
            return subprocess.run(
                command, capture_output=capture_output, text=True, cwd=self.project_root, timeout=30
            )
    ...
    def analyze_code_quality(self) -> Dict[str, Any]:
        ...
        if black_result.returncode == 0:
            ...
            quality["style_issues"] = (
                len(black_result.stdout.split("\n")) if black_result.stdout else 1
            )
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if code_quality.get("quality_score", 100) < 80:
            recommendations.append(
                "🔧 Consider running 'make format' and 'make lint' to improve code quality"
            )
        ...
        if docs.get("doc_coverage_score", 0) < 70:
            recommendations.append(
                "📚 Improve documentation coverage with more docstrings and guides"
            )
        ...
        if git.get("recent_commits", 0) < 5:
            recommendations.append(
                "⏰ Consider more frequent commits for better development tracking"
            )
    ...
    def print_dashboard(self):
        ...
        print(
            f"  • Uncommitted Changes: {'⚠️ Yes' if git.get('uncommitted_changes') else '✅ Clean'}"
        )
        ...
        print("\n🎯 Next Steps:")
        print("  • Run 'make test' to validate current state")
        print("  • Check 'make validate' for configuration issues")
        print("  • Review docs/ for latest documentation")
        print("  • Visit GitHub Actions for CI/CD status")
