    ...
    def stop_monitoring(self) -> PerformanceMetrics:
        ...
        if self._start_disk_io and PSUTIL_AVAILABLE and psutil is not None:
            try:
                disk_io = psutil.disk_io_counters()
                if disk_io:
                    disk_read_mb = (disk_io.read_bytes - self._start_disk_io['read_bytes']) / 1024 / 1024
                    disk_write_mb = (disk_io.write_bytes - self._start_disk_io['write_bytes']) / 1024 / 1024
            except Exception:
                pass
...
class PerformanceBenchmark:
    ...
    def run_comparison_benchmark(self, functions: Dict[str, Callable], *args, iterations: int = 100, **kwargs):
        ...
        return {
            ...
            'most_memory_efficient': min(comparison.keys(), key=lambda k: comparison[k]['peak_memory'])
        }
    ...
    def save_report(self, filepath: Path):
        ...
        report = {
            ...
            'system_info': {
                ...
                'memory_total_gb': (psutil.virtual_memory().total / 1024 / 1024 / 1024) if PSUTIL_AVAILABLE and psutil else 'unknown',
...
class AdapterPerformanceProfiler:
    ...
    def profile_adapter(self, adapter, examples: List[Any], batch_sizes: Optional[List[int]] = None):
        ...
        for key, metrics_list in self.profiles.items():
            if '_batch_' in key:
                ...
                if len(parts) == 2:
                    ...
                    avg_throughput = statistics.mean([m.throughput for m in metrics_list if m.throughput])
