...
class DependencyResolver:
    ...
    def resolve_dependencies(
        self, components: List[str], resolve_optional: bool = False
    ) -> Tuple[List[ComponentMetadata], List[str]]:
        """
    ...
    ) -> List[Tuple[ComponentMetadata, ComponentMetadata]]:
...
class AdvancedRegistry:
    ...
    def list_components(
        ...
        with self._lock:
            ...
            if tags:
                result = [comp for comp in result if any(tag in comp.tags for tag in tags)]
    ...
    def get_component_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for metadata in self._metadata.values():
                ...
                stats["components_by_status"][status] = (
                    stats["components_by_status"].get(status, 0) + 1
                )
                ...
                try:
                    ...
                    if not metadata.name or not metadata.import_path:
                        validation_results["errors"].append(
                            f"Component '{name}' has invalid metadata"
                        )
                    ...
                    try:
                        ...
                        validation_results["warnings"].append(
                            f"Component '{name}' import failed: {e}"
                        )
        ...
        with self._lock:
            if format == "json":
                data = {
                    ...
                    "components": {
                        name: metadata.to_dict() for name, metadata in self._metadata.items()
                    },
                }
...
def example_registry_setup():
    ...
    # Add dependencies
    qa_metadata.add_dependency(
        "exact_match", ">=1.0.0", DependencyType.REQUIRED, "Core accuracy metric"
    )
    qa_metadata.add_dependency(
        "token_f1", ">=1.0.0", DependencyType.OPTIONAL, "Enhanced F1 scoring"
    )
