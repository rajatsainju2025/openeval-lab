...
class PairedBootstrapTest(Metric):
    ...
    def compute_difference(
        self, pred1: List[Any], pred2: List[Any], references: List[Any]
    ) -> float:
    ...
    def paired_bootstrap_test(
        self, predictions1: List[Any], predictions2: List[Any], references: List[Any]
    ) -> StatisticalResult:
...
class EffectSizeMetric(Metric):
    ...
    def cohens_d(self, scores1: List[float], scores2: List[float]) -> float:
        ...
        var1 = (
            sum((x - mean1) ** 2 for x in scores1) / (len(scores1) - 1) if len(scores1) > 1 else 0
        )
        var2 = (
            sum((x - mean2) ** 2 for x in scores2) / (len(scores2) - 1) if len(scores2) > 1 else 0
        )

        pooled_std = math.sqrt(
            ((len(scores1) - 1) * var1 + (len(scores2) - 1) * var2)
...
class McNemar(Metric):
    ...
    def mcnemar_test(
        ...
        if only1_correct + only2_correct == 0:
            ...
            chi_square = (abs(only1_correct - only2_correct) - 1) ** 2 / (
...
class PearsonCorrelation(Metric):
    ...
    def compute(
        self, predictions: Iterable[float], references: Iterable[float]
    ) -> Mapping[str, float]:
        ...
        return {"pearson_r": r, "p_value": p_value, "sample_size": float(n)}
