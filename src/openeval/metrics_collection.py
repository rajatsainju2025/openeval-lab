...
class EvaluationMetrics:
        ...
        for adapter_name in sorted(adapter_names):
            ...
            if perf['metric_scores']:
                ...
                for metric, scores in perf['metric_scores'].items():
                    report.append(f"  - {metric}: {scores['mean']:.3f} (latest: {scores['latest']:.3f})")
    ...
    def export_performance_data(self, output_dir: Path) -> List[Path]:
        ...
        performance_data = {
            "run_id": self.run_id,
            "adapters": {
                name: self.get_adapter_performance(name)
                for name in self.adapter_metrics.keys()
            },
            "tasks": {
                name: self.get_task_performance(name)
                for name in self.task_metrics.keys()
            }
        }
