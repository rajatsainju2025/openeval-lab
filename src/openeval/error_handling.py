...
def retry_with_config(config: RetryConfig, error_tracker: Optional[ErrorTracker] = None):
                ...
                try:
                    ...
                    if recovery_attempts < max_recovery_attempts:
                        self.error_tracker.log_error(
                            error, context={"recovery_attempt": recovery_attempts + 1}
                        )
...
def create_robust_evaluation_context():
    ...
    network_retry = RetryConfig(
        max_attempts=3, base_delay=1.0, retryable_errors=[ConnectionError, TimeoutError, OSError]
    )
