...
class BenchmarkRunner:
    ...
    def __init__(self, output_dir: Path = Path("benchmarks"), enable_profiling: bool = True):
    ...
    def run_benchmark(self, name: str, **kwargs) -> BenchmarkResult:
        ...
        # Record metrics
        record_metric(
            "benchmark_execution_time", avg_execution_time, "histogram", {"benchmark": name}
        )
        record_metric("benchmark_memory_usage", avg_memory_usage, "histogram", {"benchmark": name})
    ...
    def run_suite(self, suite_name: str, benchmark_names: List[str], **kwargs) -> BenchmarkSuite:
    ...
    def save_suite(self, suite: BenchmarkSuite):
        ...
        for benchmark in suite.benchmarks:
            ...
            if benchmark.system_info:
                ...
                benchmark_data["system_info"][
                    "timestamp"
                ] = benchmark.system_info.timestamp.isoformat()
...
class RegressionDetector:
    ...
    def detect_regressions(
        ...
        for current_benchmark in current_suite.benchmarks:
            ...
            metric_comparisons = [
                (
                    "execution_time",
                    current_benchmark.execution_time,
                    baseline_benchmark.execution_time,
                ),
                (
                    "memory_usage_mb",
                    current_benchmark.memory_usage_mb,
                    baseline_benchmark.memory_usage_mb,
                ),
            ...
            if (
                current_benchmark.throughput is not None
                and baseline_benchmark.throughput is not None
            ):
                metric_comparisons.append(
                    ("throughput", current_benchmark.throughput, baseline_benchmark.throughput)
                )

            # Calculate error rate
            current_error_rate = (
                current_benchmark.error_count
                / max(current_benchmark.success_count + current_benchmark.error_count, 1)
                * 100
            )
            baseline_error_rate = (
                baseline_benchmark.error_count
                / max(baseline_benchmark.success_count + baseline_benchmark.error_count, 1)
                * 100
            )
            ...
            for metric_name, current_value, baseline_value in metric_comparisons:
                ...
                if is_regression:
                    ...
                    logger.warning(
                        f"Regression detected in {name}.{metric_name}: "
    ...
    def _get_recommendation(self, metric_name: str, change_percent: float) -> str:
        ...
        base_recommendation = recommendations.get(
            metric_name, "Investigate performance degradation."
        )
...
class PerformanceOptimizer:
    ...
    def analyze_results(self, suite: BenchmarkSuite) -> Dict[str, List[str]]:
        ...
        recommendations = {"memory": [], "cpu": [], "latency": [], "throughput": [], "general": []}
        ...
        for benchmark in suite.benchmarks:
            ...
            error_rate = (
                ...
                / max(benchmark.success_count + benchmark.error_count, 1)
                * 100
            )
    ...
    def generate_optimization_report(
        self, suite: BenchmarkSuite, regressions: List[RegressionResult], output_path: Path
    ):
        ...
        report = {
            ...
            "system_info": (
                asdict(suite.benchmarks[0].system_info)
                if (suite.benchmarks and suite.benchmarks[0].system_info)
                else {}
            ),
        }
