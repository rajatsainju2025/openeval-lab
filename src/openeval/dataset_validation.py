...
class DatasetValidator:
    ...
    def assess_quality(self, dataset: Dataset) -> DatasetQualityReport:
        ...
        completeness_score = (
            1.0 - (empty_inputs + empty_references) / (2 * total) if total > 0 else 0
        )
        encoding_score = 1.0 - encoding_issues / total if total > 0 else 1.0

        quality_score = (
            validity_score + uniqueness_score + completeness_score + encoding_score
        ) / 4
    ...
    def validate_dataset(self, dataset: Dataset, save_report: Optional[Path] = None) -> bool:
        ...
        is_valid = (
            report.quality_score > 0.7 and report.total_examples > 0 and report.valid_examples > 0
        )

        if self.strict and not is_valid:
            raise ValueError(
                f"Dataset validation failed. Quality score: {report.quality_score:.2f}"
            )
...
def validate_jsonl_file(
    file_path: Path, required_fields: Optional[List[str]] = None
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    reference_data = data.get(
                        "reference", data.get("answer", data.get("output", ""))
                    )
                    meta_data = {
                        k: v for k, v in data.items() if k not in ["id", "input", "reference"]
                    }
