...
class ConfigValidator:
    ...
    def _create_evaluation_schema(self) -> Optional[Any]:
        ...
        class EvaluationSchema(BaseModel):
            ...
            confidence_level: float = Field(ge=0.5, le=0.999, description="Statistical confidence level")
    ...
    def _create_adapter_schema(self) -> Optional[Any]:
        ...
        class AdapterSchema(BaseModel):
            ...
            rate_limit_rpm: Optional[int] = Field(ge=1, le=10000, description="Rate limit per minute")
    ...
    def _create_dataset_schema(self) -> Optional[Any]:
        ...
        class DatasetSchema(BaseModel):
            ...
            train_test_split: Optional[float] = Field(ge=0.1, le=0.9, description="Train/test split ratio")
...
class ConfigManager:
    ...
    def load_config(
        ...
        self._file_watchers[str(config_file)] = config_file.stat().st_mtime if config_file.exists() else 0
            ...
            if len(config_path) >= 2:
                ...
                if hasattr(config, section):
                    ...
                    if hasattr(section_config, field_name):
                        ...
                        logger.debug(f"Applied env override: {env_var} -> {section}.{field_name} = {typed_value}")
        
        return config
    
    def _apply_cli_overrides(self, config: UnifiedConfig, cli_overrides: Dict[str, Any]) -> UnifiedConfig:
            ...
            if len(path_parts) >= 2:
                ...
                if hasattr(config, section):
                    ...
                    if hasattr(section_config, field_name):
                        ...
                        # Track metadata
                        config._metadata[key] = ConfigMetadata(
    ...
    def _merge_config_data(self, config: UnifiedConfig, data: Dict[str, Any], 
                          source: ConfigSource, source_path: Optional[str] = None) -> UnifiedConfig:
