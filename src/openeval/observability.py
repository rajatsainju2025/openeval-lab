...
class DistributedTracer:
    ...
    def __init__(
        self, service_name: str, jaeger_endpoint: Optional[str] = None, sample_rate: float = 0.1
    ):
        self.service_name = service_name
...
class MetricsCollector:
    ...
    def increment_counter(self, name: str, value: int = 1, tags: Optional[Dict[str, str]] = None):
        ...
        metric_point = MetricPoint(
            name=name, value=value, timestamp=datetime.now(), tags=tags, metric_type="counter"
        )
    ...
    def set_gauge(self, name: str, value: float, tags: Optional[Dict[str, str]] = None):
        ...
        metric_point = MetricPoint(
            name=name, value=value, timestamp=datetime.now(), tags=tags or {}, metric_type="gauge"
        )
    ...
    def record_histogram(self, name: str, value: float, tags: Optional[Dict[str, str]] = None):
        ...
        metric_point = MetricPoint(
            name=name, value=value, timestamp=datetime.now(), tags=tags, metric_type="histogram"
        )
        ...
        if HAS_PROMETHEUS and hasattr(self, "request_duration") and name == "request_duration":
            self.request_duration.labels(
                service=self.service_name, method=tags.get("method", "unknown")
            ).observe(value)
...
class ObservabilityPlatform:
    ...
    def __init__(
        self, config: ObservabilityConfig, service_name: str = "openeval", version: str = "1.0.0"
    ):
        self.config = config
        self.service_name = service_name
        self.version = version
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        for name, check_func in self._health_checks.items():
            try:
                ...
                health_status["checks"][name] = {"status": "pass" if is_healthy else "fail"}
                if not is_healthy:
    ...
    def trace_operation(self, operation_name: str, tags: Optional[Dict[str, Any]] = None):
        ...
        with self.tracer.span(operation_name, tags=tags) as span_id:
            ...
            self.logger.info(
                f"Starting operation: {operation_name}", operation=operation_name, span_id=span_id
            )
            ...
            try:
                ...
                self.metrics.increment_counter(
                    "operations", 1, {"operation": operation_name, "status": "success"}
                )
                ...
                duration = time.time() - start_time
                self.metrics.increment_counter(
                    "operations", 1, {"operation": operation_name, "status": "error"}
                )
        ...
        summary = {
            ...
            "logging": {
                ...
                "error_log_count": len(
                    [
                        log
                        for log in self.logger.get_recent_logs()
...
def get_observability_platform(
    config: Optional[ObservabilityConfig] = None, service_name: str = "openeval"
