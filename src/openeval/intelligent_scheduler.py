...
class PerformancePredictor:
    ...
    def predict(self, task_type: str, dataset_size: int, adapter_type: str, 
                metric_count: int) -> Optional[Dict[str, float]]:
...
class EvaluationHistory:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            """)
            conn.execute("CREATE INDEX IF NOT EXISTS idx_task_adapter ON evaluation_profiles(task_type, adapter_type)")
    ...
    def save_profile(self, profile: EvaluationProfile):
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                profile.task_type, profile.dataset_size, profile.adapter_type,
                profile.metric_count, profile.avg_latency_ms, profile.throughput_eps,
                profile.memory_usage_mb, profile.success_rate, profile.optimal_batch_size,
                profile.optimal_concurrency, profile.timestamp.isoformat()
            ))
    ...
    def get_recent_profiles(self, days: int = 30) -> List[EvaluationProfile]:
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            for row in cursor.fetchall():
                profiles.append(EvaluationProfile(
                    task_type=row[1],
                    dataset_size=row[2],
                    adapter_type=row[3],
                    metric_count=row[4],
    ...
    def get_similar_profiles(self, task_type: str, adapter_type: str, 
                           dataset_size: int, limit: int = 10) -> List[EvaluationProfile]:
...
class IntelligentScheduler:
    ...
    def _initialize_models(self):
        ...
        if len(recent_profiles) >= 10:
            ...
            if success:
                logger.info(f"Initialized ML models with {len(recent_profiles)} historical profiles")
            else:
                logger.warning("Failed to initialize ML models, using heuristics")
        else:
            logger.info(f"Insufficient historical data ({len(recent_profiles)} profiles), using heuristics")
    
    def optimize_scheduling(self, task: Task, dataset: Dataset, adapter: Adapter, 
                          metrics: List[Metric]) -> SchedulingDecision:
        ...
        logger.info(f"Optimizing schedule for {task_type} + {adapter_type} on {dataset_size} examples")
        ...
        if ml_prediction and similar_profiles:
            decision = self._combine_ml_and_heuristics(ml_prediction, similar_profiles, dataset_size)
        ...
        logger.info(f"Scheduling decision: batch_size={decision.recommended_batch_size}, "
                   f"concurrency={decision.recommended_concurrency}, "
                   f"estimated_duration={decision.estimated_duration_minutes:.1f}min")
        
        return decision
    
    def _combine_ml_and_heuristics(self, ml_pred: Dict[str, float], 
                                 profiles: List[EvaluationProfile],
                                 dataset_size: int) -> SchedulingDecision:
        """Combine ML predictions with historical heuristics."""
        ...
        memory_per_batch = predicted_memory / avg_batch_size if avg_batch_size > 0 else predicted_memory / 32
    ...
    def _heuristic_scheduling(self, profiles: List[EvaluationProfile], 
                            dataset_size: int) -> SchedulingDecision:
        ...
        estimated_duration_minutes = (dataset_size / (avg_throughput * recommended_concurrency)) / 60
    ...
    def record_evaluation_performance(self, task: Task, dataset: Dataset, 
                                    adapter: Adapter, metrics: List[Metric],
                                    performance_stats: Dict[str, Any],
                                    used_batch_size: int, used_concurrency: int):
        ...
        profile = EvaluationProfile(
            task_type=task.__class__.__name__,
            dataset_size=len(dataset) if hasattr(dataset, '__len__') else performance_stats.get('size', 1000),
            adapter_type=adapter.__class__.__name__,
            metric_count=len(metrics),
        ...
        try:
            ...
            logger.info(f"Recorded performance profile for {profile.task_type} + {profile.adapter_type}")
        ...
        for profile in profiles:
            ...
            if profile.task_type not in task_stats:
                task_stats[profile.task_type] = {
                    'count': 0, 'total_throughput': 0, 'total_latency': 0,
                    'total_memory': 0, 'success_rates': []
                }
