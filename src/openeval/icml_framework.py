...
class ICMLExperimentRunner:
    ...
    def run_comprehensive_evaluation(
        ...
        statistical_analysis = self._comprehensive_statistical_analysis(adapter_results, config)

        # Bias analysis
        bias_analysis = {}
        if config.bias_analysis:
            bias_analysis = self._comprehensive_bias_analysis(task, dataset, adapters, config)

        # Prompt sensitivity analysis
        if config.prompt_sensitivity:
            prompt_analysis = self._prompt_sensitivity_analysis(task, dataset, adapters, config)
        ...
        result = ExperimentResult(
            ...
            model_info={name: self._get_model_info(adapter) for name, adapter in adapters.items()},
    ...
    def _multi_seed_evaluation(
        ...
        seed_results = []
        tracker = ProgressTracker(len(config.random_seeds), "Multi-seed evaluation")
    ...
    def _aggregate_multi_seed_results(
        self, results: List[Dict[str, Any]], config: ExperimentConfig
    ) -> Dict[str, Any]:
        ...
        for metric_name in results[0].get("metrics", {}):
            values = [
                r["metrics"][metric_name] for r in results if metric_name in r.get("metrics", {})
            ]
            if values:
        ...
        for metric_name, values in metrics_across_seeds.items():
            if isinstance(values[0], (int, float)):
                ...
                ci = self._bootstrap_confidence_interval(values, config.confidence_level)
                aggregated["confidence_intervals"][metric_name] = ci

        return aggregated

    def _run_single_evaluation(
        self, task: Task, dataset: Dataset, adapter: Adapter, metrics: List[Union[Metric, Any]]
    ) -> Dict[str, Any]:
    ...
    def _comprehensive_statistical_analysis(
        self, adapter_results: Dict[str, Dict[str, Any]], config: ExperimentConfig
    ) -> Dict[str, Any]:
        ...
        for i, adapter1 in enumerate(adapter_names):
            for adapter2 in adapter_names[i + 1 :]:
                ...
                if metrics1 and metrics2:
                    pairwise_analysis = self._pairwise_statistical_test(metrics1, metrics2, config)
    ...
    def _comprehensive_bias_analysis(
        ...
        bias_analysis = {"positional_bias": {}, "order_effects": {}, "prompt_artifacts": {}}
        ...
        bias_analysis["order_effects"] = self._order_effects_analysis(task, dataset, adapters)

        return bias_analysis

    def _positional_bias_analysis(
        self, task: Task, dataset: Dataset, adapters: Dict[str, Union[Adapter, Any]]
    ) -> Dict[str, Any]:
    ...
    def _generate_summary_report(self, result: ExperimentResult, output_dir: Path) -> None:
        ...
        with open(report_file, "w") as f:
            ...
            if "pairwise_comparisons" in result.statistical_analysis:
                for comparison, analysis in result.statistical_analysis[
                    "pairwise_comparisons"
                ].items():
...
def create_icml_benchmark_suite() -> Dict[str, Any]:
    ...
    return {
        ...
        "reasoning": ["commonsenseqa", "piqa", "hellaswag", "arc_challenge"],
        "reading_comprehension": ["squad_v2", "ms_marco", "natural_questions"],
        "code_generation": ["humaneval", "mbpp", "apps_introductory"],
        "mathematics": ["gsm8k", "math_algebra", "math_geometry"],
