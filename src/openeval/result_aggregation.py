...
class ResultAggregator:
    ...
    def filter_runs(
        ...
        return [
            r
            for r in self.runs
            if (not task or task in r.task)
            and (not adapter or adapter in r.adapter)
            and (not dataset or dataset in r.dataset)
    ...
    def compare_adapters(
        ...
        for run in runs:
            ...
            if metric_values and adapter not in adapter_stats:
                adapter_stats[adapter] = {"values": [], "run_count": 0, "total_examples": 0}
    ...
    def generate_summary_report(self) -> Dict[str, Any]:
        ...
        # Recent activity
        recent_runs = sorted(
            [r for r in self.runs if r.timestamp], key=lambda x: x.timestamp or "", reverse=True
        )[:5]
        ...
        return {
            ...
            "recent_runs": [
                {"run_id": r.run_id, "adapter": r.adapter, "task": r.task, "timestamp": r.timestamp}
    ...
    def find_best_runs(
        self, metric_name: str, group_by: str = "task", top_k: int = 1  # task, adapter, dataset
    ...
    def detect_regressions(
        self, metric_name: str, baseline_run_id: str, threshold: float = 0.05
    ) -> List[Dict[str, Any]]:
...
def create_analysis_dashboard(aggregator: ResultAggregator, output_dir: Path):
    ...
    aggregator.export_to_csv(output_dir / "all_runs.csv", include_all_metrics=True)
