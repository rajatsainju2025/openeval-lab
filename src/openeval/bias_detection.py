...
class BiasDetector:
    ...
    def detect_positional_bias(self, n_permutations: int = 5) -> Dict[str, Any]:
        ...
        original_accuracy = sum(
            1 for p, r in zip(original_predictions, references) if p == r
        ) / len(examples)
        ...
        for perm in range(n_permutations):
            ...
            perm_accuracy = sum(
                1 for p, r in zip(permuted_predictions, references) if p == r
            ) / len(examples)
        ...
        variance = sum((acc - mean_accuracy) ** 2 for acc in position_accuracies) / len(
            position_accuracies
        )
    ...
    def run_full_analysis(
        self, prompt_variations: Optional[List[str]] = None
    ) -> BiasAnalysisResult:
        ...
        if positional_results["detected"]:
            recommendations.append(
                "Consider balanced position calibration for multiple-choice tasks"
            )
