...
class QualityReport:
    ...
    def __post_init__(self):
        ...
        if self.metrics:
            ...
            if passed_metrics:
                self.overall_score = sum(1.0 if m.passed else 0.0 for m in passed_metrics) / len(passed_metrics)
...
class DataQualityAssessor:
    ...
    def _assess_completeness(
        ...
        if completeness < self.thresholds["completeness"]:
            ...
            issues.append(f"Found {missing_count} incomplete samples ({missing_count/len(samples):.1%})")
    ...
    def _assess_uniqueness(
        ...
        if overall_uniqueness < self.thresholds["uniqueness"]:
            ...
            issues.append(f"Found {duplicate_inputs} duplicate inputs and {duplicate_pairs} duplicate pairs")
    ...
    def _assess_relevance(
        ...
        if relevance < self.thresholds["relevance"]:
            ...
            recommendations.append("Review and remove samples with mismatched input-reference pairs")
    ...
    def _assess_balance(
        ...
        if reference_categories:
            ...
            proportions = {cat: count/total_samples for cat, count in reference_categories.items()}
    ...
    def _assess_text_quality(
        ...
        for sample in samples:
            ...
            if isinstance(sample.input, str):
                ...
                punct_ratio = sum(1 for c in input_text if not c.isalnum() and not c.isspace()) / len(input_text)
        ...
        if text_quality < 0.9:  # 90% threshold for text quality
            ...
            recommendations.append("Clean text data to remove encoding issues and excessive repetition")
