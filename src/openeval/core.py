        ...
        *,
        ...
        try:
            ...
            if concurrency > 1:
                ...
                optimal_concurrency = min(
                    concurrency, cpu_count * 2, max(1, int(available_memory / 2))
                )
                if optimal_concurrency != concurrency:
                    print(
                        f"Adjusting concurrency from {concurrency} to {optimal_concurrency} based on system resources"
                    )
        ...
        payload: Dict[str, Any] = {
            ...
            "timing": {
                ...
                "memory_usage_mb": (
                    {"initial": initial_memory, "peak": peak_memory, "current": _get_memory_usage()}
                    if initial_memory > 0
                    else None
                ),
            },
        ...
        try:
            ...
            freeze = (
                _sp.check_output([sys.executable, "-m", "pip", "freeze"], stderr=_sp.DEVNULL)
                .decode()
                .splitlines()
            )
